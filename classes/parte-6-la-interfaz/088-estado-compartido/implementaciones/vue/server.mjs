...
createServer(async (peticion, respuesta) => {
  ...
  if (url.pathname === "/por-propiedades") {
    html(pagina(await render(h(PorPropiedades, { usuario }))));
    return;
  }

  if (url.pathname === "/por-almacen") {
    escribir(usuario);
    html(pagina(await render(h(PorAlmacen))));
    return;
  }

  if (url.pathname === "/dos-ramas") {
    escribir(usuario);
    html(pagina(await render(h(DosRamas))));
    return;
  }
  ...
  if (url.pathname === "/escribir") {
    ...
    const despues = leer();
    const marcado = await render(h(DosRamas));
    json({
      antes,
      despues,
  ...
  if (url.pathname === "/coste.json") {
    ...
    json({
      ...
      ...COSTE,
      firmas_que_aceptan_el_dato: (fuente.match(/usuario: \{ type: String/g) ?? []).length,
      como_se_comparte_sin_propiedades: "provide/inject, o un almacén como Pinia",
      ...
      nota:
        "en Vue el coste se ve más porque las propiedades se declaran: la lista de lo que el componente acepta incluye un dato que no le sirve",
    });
    return;
  }
